function z_gate= gate_meas_ekf(z,gamma,model,m,P, sensor_pos)
...
for j=1:plength
        [C_ekf,U_ekf]= model.H(m(:,j), sensor_pos);
        ...
        nu= z- repmat(gen_observation_fn(model,m(:,j),zeros(size(model.D,2),1), sensor_pos),[1 zlength]);
