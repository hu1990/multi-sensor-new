function [qz_update,m_update,P_update] = ekf_update_multiple(z,model,m,P, hx, H, sensor_pos)
...
for idxp=1:plength
        [qz_temp,m_temp,P_temp] = ekf_update_single(z,model,m(:,idxp),P(:,:,idxp), hx, H, sensor_pos);
       qz_update(idxp,:)   = qz_temp;
        m_update(:,idxp,:) = m_temp;
        P_update(:,:,idxp) = P_temp;
end

function [qz_temp,m_temp,P_temp] = ekf_update_single(z,model,m,P, hx, H, sensor_pos)

eta = gen_observation_fn(model,m,'noiseless', hx, sensor_pos);

[H_ekf,U_ekf]= H(m, sensor_pos);                 % user specified function for application
